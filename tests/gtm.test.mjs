...
test('9-agent GTM studio returns all agents in order', async () => {
  ...
  assert.equal(ids.length, 9, 'expected 9 agents, got ' + ids.length);
  assert.deepEqual(ids, ['strategist', 'researcher', 'icp', 'offer', 'channel', 'message', 'skeptic', 'planner', 'publisher']);
});
